...
import { Plus, Trash2, FileText, Check, Pencil, RotateCcw } from "lucide-react";
...
function CustomerInvoicesPage() {
  ...
  const postMut = useMutation({
    ...
    onSuccess: () => {
      toast.success(t("invoices.posted"));
      qc.invalidateQueries({ queryKey: ["invoices"] });
    },
    onError: (e: Error) => toast.error(formatLockError(e, t)),
  });
                ...
                <table className="w-full text-xs">
                        ...
                        <tr key={i} className="border-t">
                          ...
                          <td className="p-1">
                            <Select value={l.account_id} onValueChange={(v) => { const n = [...lines]; n[i] = { ...l, account_id: v }; setLines(n); }}>
                              ...
                              <SelectContent>
                                {incomeAccounts.map((a: any) => <SelectItem key={a.id} value={a.id}>{a.code} — {localized(a, "name")}</SelectItem>)}
                              </SelectContent>
                            </Select>
                          </td>
                          ...
                          <td className="p-1">
                            <Select value={l.tax_id} onValueChange={(v) => {
                              const tax = salesTaxes.find((tx: any) => tx.id === v);
                              const n = [...lines]; n[i] = { ...l, tax_id: v, tax_rate: tax ? Number(tax.rate) : 0 }; setLines(n);
                            }}>
                              <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="—" /></SelectTrigger>
                              <SelectContent>
                                <SelectItem value="none">— {t("invoices.noTax")} —</SelectItem>
                                {salesTaxes.map((tx: any) => <SelectItem key={tx.id} value={tx.id}>{tx.code} ({tx.rate}%)</SelectItem>)}
                              </SelectContent>
                            </Select>
                          </td>
                  ...
                  <tfoot className="bg-muted/30 font-mono">
                    <tr className="border-t">
                      <td colSpan={5} className="p-2 text-end font-medium">{t("invoices.subtotal")}</td>
                      <td className="p-2 text-end">{fmt(totals.subtotal)}</td><td></td>
                    </tr>
        ...
        <table className="w-full text-xs">
          <thead className="bg-muted/50">
            <tr>
              ...
              <th className="text-start p-3 font-medium">{t("invoices.customer")}</th>
